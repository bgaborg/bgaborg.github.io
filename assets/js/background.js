...
class MorphingBackground {
    ...
    transformParticles(scaleX, scaleY) {
    ...
    transformGravityPoints(scaleX, scaleY) {
    ...
    updateParticle(p) {
        ...
        if (p.x < -50)
            p.baseX += this.canvas.width + 100;
        if (p.x > this.canvas.width + 50)
            p.baseX -= this.canvas.width + 100;
        if (p.y < -50)
            p.baseY += this.canvas.height + 100;
    ...
    drawParticle(p) {
        ...
        gradient.addColorStop(0.1, `rgba(${brightness}, ${brightness}, ${brightness}, ${alpha * 0.6})`);
        gradient.addColorStop(0.3, `rgba(${brightness}, ${brightness}, ${brightness}, ${alpha * 0.2})`);
        gradient.addColorStop(0.6, `rgba(${brightness}, ${brightness}, ${brightness}, ${alpha * 0.05})`);
